...
class Mitotic_Classifier(pl.LightningModule):
    ...
    def __init__(self, hparams) -> None:
        ...
        if m['name'] == 'resnet':
            ...
            self.model = generate_model(model_depth=m['depth'],
        ...
        if 'test_data' not in self.hparams:   # train
            ...
            train_filenames = glob(hparams.exp_params['training_data_path']
                                   + os.sep + '*.npy')
            self.train_filenames = train_filenames

            val_filenames = glob(hparams.exp_params['validation_data_path']
                                 + os.sep + '*.npy')
    ...
    def training_step(self, batch, batch_idx, optimizer_idx=0):
        ...
        if self.class_weight is None:
            ...
            loss = F.cross_entropy(y_hat, y, self.class_weight.cuda(),
                                   reduction='mean')
        ...
        return {'loss': loss,
                ...
                "train_n_pred": len(x), 'pred_record': pred_dict}

    def training_epoch_end(self, outputs):
        avg_loss = torch.stack([x['loss'] for x in outputs]).mean()
        train_acc = sum([x['train_n_correct_pred'] for x in outputs])  \
            / sum(x['train_n_pred'] for x in outputs)
        ...
        tensorboard_logs = {'train_epoch_loss': avg_loss,
    ...
    def validation_step(self, batch, batch_idx, optimizer_idx=0):
        ...
        if self.class_weight is None:
            ...
            val_loss = F.cross_entropy(y_hat, y, self.class_weight.cuda(),
                                       reduction='mean')
        ...
        pred_prob = y_hat.cpu().data.float()
        pred_dict = {'idx': batch_idx, 'pred': [pred_prob.numpy()],
                     'pred_label': labels_hat, 'label': y, 'fn': _fn_}
    ...
    def validation_epoch_end(self, outputs):
        ...
        csv_fn = self.exp_params['debug_path'] + 'val/' + \
            str(random.randint(1, 9000)) + '.csv'
        df.to_csv(csv_fn)
    ...
    def test_step(self, batch, batch_idx, optimizer_idx=0):
        ...
        if self.test_type == 'folder':
            ...
            pred_dict = {'CellId': cid, 'pred': pred_prob.numpy()}
    ...
    def configure_optimizers(self):
        ...
        optimizer = optim.Adam(self.model.parameters(),
                               lr=exp_m['LR'],
                               weight_decay=exp_m['weight_decay'])
        ...
        if exp_m['scheduler']['name'] is not None:
            if exp_m['scheduler']['name'] == "ExponentialLR":
                ...
                scheduler = ExponentialLR(optims[0],
                ...
                scheduler = CALR(optims[0],
                                 T_max=exp_m['scheduler']['T_max'])

            elif exp_m['scheduler']['name'] == "StepLR":
                from torch.optim.lr_scheduler import StepLR
                assert exp_m['scheduler']['step_size'] > 0
                assert exp_m['scheduler']['gamma'] > 0
                scheduler = StepLR(optims[0],
    ...
    def train_dataloader(self):
        ...
        if data_m['name'] == 'AdaptivePaddingBatch':
            ...
            train_set_loader = DataLoader(
                adaptive_padding_loader(self.train_filenames,
                                        out_shape=data_m['shape']),
    ...
    def val_dataloader(self):
        ...
        if data_m['name'] == 'AdaptivePaddingBatch':
            ...
            val_set_loader = DataLoader(
                adaptive_padding_loader(self.val_filenames,
                                        out_shape=data_m['shape'],
                                        test_flag='F'),
                batch_size=data_m['batch_size'],
                num_workers=data_m['num_worker'])
    ...
    def test_dataloader(self):
        ...
        if self.test_type == 'df':
            assert data_m['name'] == 'AdaptivePaddingBatch', \
                   'only adaptive paddng loader is support when using csv'
            from ..data_loader.universal_loader import adaptive_padding_loader
            test_set_loader = DataLoader(
                adaptive_padding_loader(data_t['data_path'],
                                        out_shape=data_m['shape'],
                                        test_flag='C'),
            ...
            if data_m['name'] == 'AdaptivePaddingBatch':
                from ..data_loader.universal_loader \
                    import adaptive_padding_loader
                test_set_loader = DataLoader(
                    adaptive_padding_loader(filenames,
                                            out_shape=data_m['shape'],
                                            test_flag='F'),
                    batch_size=data_t['batch_size'],
                    num_workers=data_t['num_worker'])
