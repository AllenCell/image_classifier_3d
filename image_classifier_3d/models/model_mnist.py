...
class LightningMNISTModel(LightningModule):
    """
    ...
    def prepare_data(self):
        ...
        self.mnist_train = MNIST(
            self.hparams.data_root, train=True, download=True, transform=transform
        )
        self.mnist_test = MNIST(
            self.hparams.data_root, train=False, download=True, transform=transform
        )

    def train_dataloader(self):
        log.info("Training data loader called.")
        return DataLoader(
            self.mnist_train, batch_size=self.hparams.batch_size, num_workers=4
        )

    def val_dataloader(self):
        log.info("Validation data loader called.")
        return DataLoader(
            self.mnist_test, batch_size=self.hparams.batch_size, num_workers=4
        )
