...
class ViT_3D(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        assert (
            np.any([a % b for a, b in zip(image_size, patch_size)]) == 0
        ), "Image dimensions must be divisible by the patch size."
        num_patches_per_dim = [a // b for a, b in zip(image_size, patch_size)]
        ...
        self.to_patch_embedding = nn.Sequential(
            Rearrange(
                "b c (z p0) (h p1) (w p2) -> b (z h w) (p0 p1 p2 c)",
                p0=patch_size[0],
                p1=patch_size[1],
                p2=patch_size[2],
            ),
            # TODO: change linear embedding to CNN based embedding
