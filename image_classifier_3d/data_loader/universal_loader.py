...
class adaptive_padding_loader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if img.shape[1] > self.out_shape[0] or \
           img.shape[2] > self.out_shape[1] or \
           img.shape[3] > self.out_shape[2]:
            scaling_ratio = min([self.out_shape[0] / img.shape[1],
                                self.out_shape[1] / img.shape[2],
                                self.out_shape[2] / img.shape[3]])
            img_temp = []
            for ch in range(img.shape[0]):
                img_temp.append(zoom(img[ch, :, :, :], scaling_ratio,
                                order=1))
...
class adaptive_loader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        new_img = np.zeros((img.shape[0],
                            img.shape[1],
                            tmp.shape[0],
                            tmp.shape[1]), dtype=np.float32)
        for ch in range(img.shape[0]):
            for zz in range(img.shape[1]):
                new_img[ch, zz, :, :] = ndi.rotate(img[ch, zz, :, :],
                                                   rand_angle,
                                                   reshape=True,
                                                   order=2)
